...
- F3.3 网页端导出（预生成路线，已实现）：`pnpm prebuild:pdf` 生成全量矩阵（整站 + 每章节 + 每页面 × 全部模板）与 `manifest.json` 到 `content/public/pdfs/`，随站点构建一同部署。交互入口：
  - **下载中心页** `/export`：阅读器式独立页面，脱离文档版式（`sidebar: false` + `aside: false` + 全宽 pageClass，仅保留顶栏）——左侧下载目录吸顶（整站置顶、章节可折叠、条目缩进），右侧大面积内嵌预览 + 模板分段切换 + 下载按钮；移动端目录折叠为「选择内容」按钮面板，选择后自动收起；支持 URL 参数直达（`/export?target=graph&tpl=double`）；
