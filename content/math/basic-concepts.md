...
关于公约数、公倍数、最大公约数与最小公倍数，四个名词的定义，见 [最大公约数](../basics/gcd.md)。
...
互素的性质与最大公约数理论：裴蜀定理（Bézout's identity）。见 [裴蜀定理](./bezout.md)。
...
> [!TIP]
> 该定理等价于证明 Euler 函数为 [积性函数](./arithmetic-function.md#积性函数)。
...
> [!WARNING] 待补充：原文含脚注引用 `[^gcdcpp]` 与 `[^ref1]`，但未给出脚注定义，需补充来源。
